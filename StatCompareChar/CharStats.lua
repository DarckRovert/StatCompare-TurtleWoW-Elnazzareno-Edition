...
function StatCompare_CharStats_Scan(bonuses, unit)
	...
	if(class == "DRUID") then
		if(not CharStats_fullvals["BEARAP"]) then
			...
			CharStats_fullvals["BEARAP"] = CharStats_fullvals["BEARAP"] + CharStats_fullvals["ATTACKPOWER"] + (60 * 3);
		end
		if(not CharStats_fullvals["CATAP"]) then
			CharStats_fullvals["CATAP"] = CharStats_fullvals["ATTACKPOWER"] + (60 * 2) + CharStats_fullvals["AGI"];
		else
			CharStats_fullvals["CATAP"] = CharStats_fullvals["CATAP"] + CharStats_fullvals["ATTACKPOWER"] + (60 * 2) + CharStats_fullvals["AGI"];
		end
	else
	...
	if( class == "DRUID") then
		if(StatCompare_SpellStats_CheckTalents) then
			...
			if(tolents > 0) then
				if(not CharStats_fullvals["MANAREG"]) then
					CharStats_fullvals["MANAREG"] = CharStats_fullvals["MANAREGSPI"] * tolents * 2.5;
				else
					CharStats_fullvals["MANAREG"] = CharStats_fullvals["MANAREG"] + CharStats_fullvals["MANAREGSPI"] * tolents * 2.5;
				end
				CharStats_fullvals["MANAREG"] = math.ceil(CharStats_fullvals["MANAREG"]);
			end
		end
	elseif( class == "PRIEST") then
		if(StatCompare_SpellStats_CheckTalents) then
