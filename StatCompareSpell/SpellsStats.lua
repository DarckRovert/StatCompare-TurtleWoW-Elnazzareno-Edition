...
-- Druid
  ...
  [SC_TALENT_INTENSITY] = {type="n", rank=3, rankratio={[0]=0.0;[1]=0.05;[2]=0.10;[3]=0.15;}; texture="Interface\\Icons\\Spell_Frost_WindWalkOn"};
-- Priest
  ...
  [SC_TALENT_MEDITATION] = {type="n", rank=3, rankratio={[0]=0.0;[1]=0.05;[2]=0.10;[3]=0.15;}; texture = "Interface\\Icons\\Spell_Nature_Sleep"};
-- Shaman
	...
	if(SpellName and SpellRank and SC_SpellRanks[SpellName] and SC_SpellRanks[SpellName][SpellRank]) then
		...
		if(tinfos) then
			if(tinfos.ratios) then -- Ratio
				for _,tid in pairs(tinfos.ratios) do
			...
			if(SC_SpellRanks[SC_SpellCollections[SpellName].d_h_ot][SpellRank]) then
				...
				if(tmpinfos) then
					if(tmpinfos.ratios) then -- Ratio
						for _,tmpid in pairs(tmpinfos.ratios) do
...
function StatCompare_SpellStats_CheckTalents(talentids)
	local numTabs = GetNumTalentTabs();
	for t=1, numTabs do
		local numTalents = GetNumTalents(t);
		for i=1, numTalents do
			local name,icon,x,y,rank,max = GetTalentInfo(t,i);
			if(SC_Talents[talentids] and SC_Talents[talentids].texture == icon) then
				return SC_Talents[talentids].rankratio[rank];
			end
		end
	end
	return 0;
